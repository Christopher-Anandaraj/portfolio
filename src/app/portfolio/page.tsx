        ...
        {projects.map((project, idx) => (
          <div key={idx} className="bg-[#23262b] border border-[#2d2f36] rounded-2xl p-0 flex flex-col min-h-[400px] w-full shadow-2xl overflow-hidden">
            ...
            <div className="flex flex-col gap-6 p-10 flex-1">
              ...
              <span className="text-lg text-gray-400 mb-2">{project.category}</span>
              {project.description && (
                <p className="text-base text-gray-300 mb-2">{project.description}</p>
              )}
              {project.hosted !== undefined && (
                <div className="text-sm text-gray-400 mb-2">Hosted: <span className={`font-semibold ${project.hosted ? 'text-green-400' : 'text-red-400'}`}>{project.hosted ? 'True' : 'False'}</span></div>
              )}
              <a href={project.link} target="_blank" rel="noopener noreferrer" className="text-blue-400 text-xl font-semibold hover:underline self-start mt-auto">View project →</a>
            </div>
          </div>
        ))}
      </div>
